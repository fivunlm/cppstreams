...
TEST( Stream, ReduceConst ) {
	...
	{int result = stream( values )->reduce( static_cast<std::function<int( const int&, const int& )>>([]( const int& val, const int& difference ) { return difference - val; }), 28 ); EXPECT_EQ( result, 0 ); }
